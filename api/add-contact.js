⏺ export default async function handler(req, res) {                             
    ...
    try {                                                                       
      const { email, firstName, breachCount, breachNames, country } = req.body
  ...
  fetch('https://idpmgroup.api-us1.com/api/3/contacts', {                       
        method: 'POST',
        headers: {
      ...
      if (!createContactResponse.ok) {                                          
        ...
        console.error('Contact creation error:', createContactResponse.status,  
  errorText) 
      ...
      // STEP 2: Subscribe to List 9 (all scanner contacts, US + Canada)        
      const addToListResponse = await
  fetch('https://idpmgroup.api-us1.com/api/3/contactLists', {                   
        method: 'POST',
        headers: {                                                              
          'Api-Token': process.env.AC_API_KEY,
          'Content-Type': 'application/json'                                    
      ...
      if (!addToListResponse.ok) {
        ...
        console.error('List subscription error:', addToListResponse.status,
  errorText)                                                                    
